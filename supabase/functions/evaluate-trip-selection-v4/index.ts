...
function questions(item:Item,decision:string){
 if(decision!=="review"&&decision!=="restricted"&&decision!=="conditional")return [];
...
Deno.serve(async req=>{
 ...
 try{
   ...
   if(!rawItems.length)return json({ok:false,error:"Select at least one item"},400);
   ...
   const userRes=await client.auth.getUser();const user=userRes.data.user;if(!user)return json({ok:false,error:"Session expired"},401);
   const sourceItems:Array<Obj>=Array.isArray(data.items)?data.items:[];
   const items=sourceItems.map((r,i)=>{
     const original=rawItems[i]||{} as Item;const qs=questions(original,r.decision);
   ...
   const metadataUpdates=rawItems.map(async original=>{
     const payload={answers_v4:original.answers||{},item_weight_grams_v4:Number(original.weightGrams||0)||null,declaration_name_en_v4:original.declarationNameEn||null};
     return client.from("trip_items").update(payload).eq("trip_id",body.tripId).eq("user_id",user.id).eq("catalog_key_v3",original.catalogKey);
   });
   ...
   const sourceMap=new Map<string,Obj>();for(const i of declarationItems)for(const s of i.sources||[])if(s?.url)sourceMap.set(s.url,s);
   if(declarationItems.length){const snap=await client.from("trip_declaration_snapshots_v4").insert({trip_id:body.tripId,user_id:user.id,catalog_version:body.catalogVersion||"2026.08.30-v4",engine_version:VERSION,items:declarationItems,source_snapshot:[...sourceMap.values()]});if(snap.error)console.warn("EntrySafe V4 declaration snapshot warning",snap.error.message);}

   return json({ok:true,engineVersion:VERSION,officialEngineVersion:data.engineVersion||data.version||"v3",summary,items,aggregateChecks:data.aggregateChecks||[],pendingQuestions:items.flatMap(i=>(i.questions||[]).map((q:Obj)=>({catalogKey:i.catalogKey,name:i.name,question:q}))),sources:[...sourceMap.values()]});
