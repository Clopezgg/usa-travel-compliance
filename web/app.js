...
async function deleteTrip(){if(!confirm('¿Eliminar este viaje y todos sus registros asociados?'))return; const id=state.selectedTrip.id; const {error}=await supabase.from('trips').delete().eq('id',id); if(error)throw error; state.selectedTrip=null; state.bags=[]; state.items=[]; state.documents=[]; await loadTrips(); await loadDashboardItems(); renderTrips(); renderDashboard(); renderTripWorkspace(); switchTab('trips');}
